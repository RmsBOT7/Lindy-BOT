	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Semoga Tenang Di Alam Sana @${num.split('@')[0]}👋 Al-fatihah`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			switch(command) {
				...
				case 'info':
					...
					teks = `*Nama bot* : ${me.name}\n*Author* : *XPTN*\n*Nomor Bot* : @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*Total Block Contact* : ${blocked.length}\n*The bot is active on* : ${kyun(uptime)}\n\n*Device* : Realme 7pro\n*Sinyal* : Indosat asuu\n*Baterai* : Low ngentod!\n\nUntuk menghindari Bot Pending/delay diharapkan tidak spam`
			...
			case 'owner':
				...
				case 'watercolor':
					...
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/watercolour_text?text1=${tels}&apikey=ANTIGRATISNIHANJENKKK`, {method: 'get'})
               ...
               case 'infogempa':
				...
				case 'elang':
					...
					break
				case 'babi':
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=babi&apikey=ANTIGRATISNIHANJENKKK`, {method: 'get'})
